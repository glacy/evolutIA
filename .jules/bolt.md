...
## 2025-05-25 - Caching Bugs and Dictionary Lookups in Extraction
**Learning:** Returning objects too early from a parsing block bypasses caching logic. Furthermore, extracting values from an array by manually matching keys within an outer loop generates $O(N \times M)$ complexity which causes slowdowns parsing large directories.
**Action:** Always verify a return statement isn't bypassing caching logic. Convert iterative array lookups to dictionary lookups ($O(N+M)$) when iterating over related subsets.
