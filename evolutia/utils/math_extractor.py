...
def extract_math_expressions(content: str) -> List[str]:
    ...
    # Use findall for faster extraction (avoids Match object creation overhead)
    for groups in COMBINED_MATH_PATTERN.findall(content):
        # groups is a tuple of strings corresponding to capturing groups.
        # Find the first non-empty group.
        expr = groups[0] or groups[1] or groups[2] or groups[3] or groups[4]
        if expr:
...
def extract_variables(math_expressions: List[str]) -> Set[str]:
    ...
    for expr in math_expressions:
        ...
        for groups in COMBINED_VARIABLES_PATTERN.findall(expr):
            # Iterate through groups to find the non-empty match
            for var in groups:
                if var:
                    variables.add(var)
                    break
