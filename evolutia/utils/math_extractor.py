...
GREEK_REGEX = (
    r"\\(alpha|beta|gamma|delta|epsilon|theta|phi|rho|omega|sigma|lambda|mu|nu|pi|tau)"
)
...
MATRICES_PATTERN = re.compile(
    r"\\begin\{matrix\}|\\begin\{pmatrix\}|\\begin\{bmatrix\}"
)
...
def extract_math_expressions(content: str) -> List[str]:
    ...
    # Use findall for faster extraction (returns tuples of groups)
    # Groups: 0:block, 1:display_dollar, 2:display_bracket, 3:inline_dollar, 4:inline_paren
    for groups in COMBINED_MATH_PATTERN.findall(content):
        # Check inline_dollar (index 3) first as it's most common
        expr = groups[3] or groups[1] or groups[2] or groups[0] or groups[4]
        if expr:
            expressions.append(expr.strip())

    logger.debug(
        f"[MathExtractor] Extraídas {len(expressions)} expresiones matemáticas del contenido"
    )
    return expressions
...
def extract_variables(math_expressions: List[str]) -> Set[str]:
    ...
    for expr in math_expressions:
        # Use findall for faster extraction
        # Groups: 0:vec, 1:mathbf, 2:hat, 3:plain, 4:greek
        for groups in COMBINED_VARIABLES_PATTERN.findall(expr):
            # Check plain (index 3) first as it's most common
            var = groups[3] or groups[4] or groups[0] or groups[1] or groups[2]
            if var:
                variables.add(var)

    logger.debug(
        f"[MathExtractor] Extraídas {len(variables)} variables de {len(math_expressions)} expresiones"
    )
    return variables
...
def estimate_complexity(expressions: List[str]) -> float:
    ...
    logger.debug(
        f"[MathExtractor] Complejidad estimada: {total_complexity:.2f} (de {len(expressions)} expresiones)"
    )
