...
class MaterialExtractor:
    ...
    def get_all_exercises(self, materials: List[Dict]) -> List[Dict]:
        ...
        for material in materials:
            # Optimización: Lookup table para soluciones (O(1) en vez de O(N))
            solutions_lookup = {sol['exercise_label']: sol for sol in material.get('solutions', [])}

            for exercise in material.get('exercises', []):
                # Buscar solución correspondiente (O(1))
                solution = solutions_lookup.get(exercise['label'])
